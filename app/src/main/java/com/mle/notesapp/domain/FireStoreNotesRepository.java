...
public class FireStoreNotesRepository implements NotesRepository {
    ...
    public void add(String title, String message, Callback<Note> callback) {
        ...
        fireStore.collection(NOTES).add(data).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
            @Override
    ...
    public void update(Note note, String title, String message, Callback<Note> callback) {
        ...
        fireStore.collection(NOTES)
                ...
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void unused) {
                        Note updatedNote = new Note(note.getId(), title, message, note.getCreatedAt());
